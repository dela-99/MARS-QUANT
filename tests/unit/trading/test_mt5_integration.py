...
class TestMT5DemoIntegration:
    ...
    def test_end_to_end_mock(self, tmp_path):
        ...
        mock_mt5.account_info.return_value = Mock(
            login=476944496, trade_mode=0, balance=100000.0, equity=100000.0,
            currency="USD", leverage=2000
        )
        mock_mt5.symbol_info.return_value = Mock(
            name="XAUUSDm", trade_contract_size=100.0, volume_min=0.01, volume_max=200.0,
            volume_step=0.01, digits=3, point=0.001, spread=260,
            bid=4414.36, ask=4414.62, visible=True,
            trade_tick_size=0.001, trade_tick_value=1.0,
            swap_long=-1.5, swap_short=-0.5,
            margin_initial=1000.0, margin_maintenance=500.0,
            session_deals=0, session_buy_orders=0, session_sell_orders=0,
            time=1704067200
        )
        mock_mt5.symbol_info_tick.return_value = Mock(bid=4414.36, ask=4414.62)
        mock_mt5.order_send.return_value = Mock(
            retcode=10009, ticket=123456789, order=987654321,
            volume=0.5, price=4414.62, sl=4400.0, tp=4450.0, comment="MARS_1_100000"
        )
        mock_mt5.ORDER_TYPE_BUY = 0
        mock_mt5.ORDER_TYPE_SELL = 1
        mock_mt5.TRADE_ACTION_DEAL = 1
