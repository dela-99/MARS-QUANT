...
CONFIG = {
    ...
    "log_file": "logs/hyp_a_xgb_trading_bot.log"
}
...
def get_trade_signal():
    ...
    try:
        # TODO: create data preprocessing pipeline class for hyp a
        # 1. Get latest data (needs enough for indicator warm-up)
        rates = mt5.copy_rates_from_pos(CONFIG["symbol"], mt5.TIMEFRAME_H1, 0, 300)
        ...
        features = {
            ...
            'atr_at_asia_close': df.loc[end_of_asia_ts]['ATRr_14'],
            'rsi_at_asia_close': df.loc[end_of_asia_ts]['RSI_14'],
            'ema50_dist': (asia_close - df.loc[end_of_asia_ts]['EMA_50']) / df.loc[end_of_asia_ts]['EMA_50'],
            'ema200_dist': (asia_close - df.loc[end_of_asia_ts]['EMA_200']) / df.loc[end_of_asia_ts]['EMA_200'],
        }
        ...
        return int(prediction) # Returns 0 or 1

    except Exception as e:
        logging.error(f"An error occurred in get_trade_signal: {e}")
        return None
