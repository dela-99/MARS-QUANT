...
COLUMN_ALIASES: Mapping[str, str] = {
    ...
    "datetime": "timestamp",
    "Date": "timestamp",
    # "date": "timestamp",  # DISABLED: conflicts with existing "timestamp" column in some CSVs
    "Open": "open",
    "High": "high",
    "Low": "low",
    "Close": "close",
...
class OHLCVNormalizer(DataNormalizer):
    ...
    def normalize(self, df: pd.DataFrame, timezone: str = "UTC") -> pd.DataFrame:
        ...
        if "timestamp" in frame.columns:
            # Check if timestamp is already a DatetimeIndex
            if isinstance(frame.index, pd.DatetimeIndex) and frame.index.name == "timestamp":
                # Already set, skip
                pass
            else:
